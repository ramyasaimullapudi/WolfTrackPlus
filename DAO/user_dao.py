...
class user_dao:
    ...
    def get_user(self, email, password):
        ...
        result = self.__db.run_query(
            "SELECT count(*),user_id,email,full_name,gender,location FROM user WHERE email='"+email+"'")
    ...
    def update_details(self, user_id, name, gender, location):
        res = self.__db.run_query(
            "UPDATE user SET full_name = '" + name + "',gender = '" + gender + "',location = '" + location + "'WHERE user_id=" + str(user_id))
        print(res)
        return res
